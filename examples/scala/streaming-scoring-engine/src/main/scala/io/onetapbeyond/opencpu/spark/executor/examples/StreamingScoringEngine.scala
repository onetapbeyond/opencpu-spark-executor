...
object StreamingScoringEngine {
	...
	def main(args:Array[String]):Unit = {
		...
		try {
			 ...
			 */			
		    val endpoint = sc.broadcast(EX_OCPU_SERVER)
			...
			rResultStream.foreachRDD { rdd => {
				rdd.foreach { result => {
					println("StreamingScoringEngine: " +
						"tvscore::tv input=" + result.input + 
			 ...
			 * to EX_APP_TIMEOUT so we do not overload the public
			 * OpenCPU server used by default in this application.
			 */
			ssc.awaitTerminationOrTimeout(EX_APP_TIMEOUT)
	...
	def initSparkContext():SparkContext = {
		val conf = new SparkConf().setAppName(EX_APP_NAME)
		new SparkContext(conf)
	}

	def initSparkStreamingContext(sc:SparkContext):StreamingContext = {
	    new StreamingContext(sc, Seconds(EX_STREAM_BATCH_DURATION))
	}

	private val EX_OCPU_SERVER = "http://public.opencpu.org/ocpu"
	private val EX_APP_NAME = "ROSE Streaming Scoring Engine Example"
	private val EX_STREAM_BATCH_DURATION = 1
	private val EX_STREAM_DATA_PER_SECOND = 3
	private val EX_APP_TIMEOUT = 10000
